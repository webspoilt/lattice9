...
<img src="https://raw.githubusercontent.com/webspoilt/lattice9/main/docs/diagrams/00-architecture-overview.png" alt="Lattice9 Architecture Overview" width="100%"/>
...
<img src="https://raw.githubusercontent.com/webspoilt/lattice9/main/docs/diagrams/01-core-architecture-topology.png" alt="Core Architecture Topology" width="100%"/>
...
<img src="https://raw.githubusercontent.com/webspoilt/lattice9/main/docs/diagrams/02-bayesian-evidence-dag.png" alt="Bayesian Evidence Propagation DAG" width="100%"/>
...
<img src="https://raw.githubusercontent.com/webspoilt/lattice9/main/docs/diagrams/07-attack-path-synthesis.png" alt="Attack Path Synthesis" width="100%"/>
...
<img src="https://raw.githubusercontent.com/webspoilt/lattice9/main/docs/diagrams/03-attack-path-topology.png" alt="Attack Path Topology with Resistance" width="100%"/>
...
The adversarial cost to reach critical asset $\nu$ from entry point $A$ is computed as:

$$C(A \to \nu) = \sum_{i} \frac{c_i}{R_{\text{topo}}(e_i)}$$

Where $c_i$ is the operational cost of exploiting edge $e_i$ and $R_{\text{topo}}(e_i)$ is the topology resistance of that edge. The minimum-cost path identifies the adversary's optimal attack investment:

$$C_{\min}(A_1 \to \nu) = \text{\$3.2M} \quad | \quad \text{ROI} = \frac{V(\nu)}{C_{\min}} = 2.1\times$$

When the ROI exceeds the threat threshold, Lattice9 escalates the defense posture for the affected subgraph.
...
Every finding in Lattice9 carries a recursive cryptographic provenance chain. The evidence lineage module constructs a directed acyclic graph where each node represents a finding and edges represent derivation relationships:

$$\text{lineage}(f) = \{(f_i, f) : f \text{ derived from } f_i\}$$

Each finding stores:
- **Source hash** — SHA-256 of the raw evidence
- **Derivation proof** — Hash chain from ancestors
- **Confidence propagation** — Bayesian posterior given parent evidence
- **Temporal validity** — Exponential decay from observation time

This enables **complete auditability**: any finding can be traced back through its derivation chain to the original telemetry, with confidence scores updated at each hop.

---

## Information Geometry

### Geodesic Path Routing

Lattice9 treats the attack surface as a Riemannian manifold where the metric tensor $g_{ij}$ encodes traversal resistance:

$$ds^2 = g_{ij}\, dx^i\, dx^j$$

The geodesic distance between two nodes on this manifold is:

$$d_\gamma(v_i, v_j) = \min_\sigma \int_\sigma \sqrt{g_{ij}\, dx^i\, dx^j}$$

Where $\sigma$ ranges over all paths from $v_i$ to $v_j$. Geodesic paths on the risk manifold trace the least-resistance routes — topologically shortest paths that simultaneously minimize detection probability and operational cost.
...
The graph Laplacian $L = D - A$ (where $D$ is the degree matrix and $A$ is the adjacency matrix) provides the spectral foundation for topology resilience assessment:

$$Lf = \lambda f$$

The second-smallest eigenvalue $\lambda_2$ (the Fiedler value) quantifies algebraic connectivity:

$$\lambda_2(G) = 2.41 \implies G \text{ is 2-edge-connected}$$

The **Cheeger inequality** bounds the isoperimetric constant $h(G)$:

$$\frac{\lambda_2}{2} \leq h(G) \leq \sqrt{2\lambda_2}$$

This provides a mathematically rigorous measure of how resistant the topology is to partition attacks — the minimum cut that disconnects the graph.
...
│   │   ├── schema.py                   # Neo4j schema + indexes
│   │   ├── engine.py                   # Transactional CRUD operations
│   │   ├── algorithms.py               # Constraint-aware Dijkstra traversals
│   │   ├── temporal.py                 # Snapshot generation + drift
│   │   ├── confidence.py               # Bayesian confidence propagation
│   │   ├── evolution.py                # Instability + entropy tracking
│   │   ├── field_theory.py             # Attack pressure gravity wells
│   │   ├── resistance.py               # Topological monitoring friction
│   │   ├── wave_propagation.py         # Threat wave diffusion
│   │   ├── topological_da.py           # Persistent homology + Betti numbers
│   │   ├── gnn_reasoning.py            # Node2Vec structural embeddings
│   │   ├── attractor_theory.py         # Compromise convergence basins
│   │   ├── information_geometry.py     # Geodesic path routing
│   │   └── blast.py                    # Multi-dimensional blast radius
│   │
│   ├── reasoning/                      # Analytical path planning
│   │   ├── attack_paths.py             # Multiclass attack path planners
│   │   ├── exploit_chains.py           # Exploit prerequisite solvers
│   │   ├── prioritization.py           # Contextual node prioritization
│   │   ├── counterfactual.py           # Sandboxed mitigation simulation
│   │   ├── entropy.py                  # Path uncertainty collapse
│   │   ├── causal.py                   # Structural Causal Models
│   │   ├── adversarial_game.py         # Minimax equilibrium calculators
│   │   └── attack_economics.py         # Attacker ROI financial scoring
│   │
│   ├── evidence/                       # Evidence and provenance
│   │   └── lineage.py                  # Cryptographic provenance tracing
│   │
│   └── proxima/                        # Proxima MCP + agent execution
...
│   └── src/components/
│       ├── IntelligencePanel.tsx       # Tactical operator interface
...
└── docs/                               # Publication-Grade Specifications
    ...
    │   └── operational-deployment.md   # Hardening + air-gapped setup
    ├── architecture/
    │   └── distributed-worker-architecture.md
    └── research/
        └── engineering-blog-autopsy.md
```
...
|:---------|:------------|
| [Systems Whitepaper](docs/whitepaper/lattice9-whitepaper.md) | Comprehensive mathematical and architectural specification |
| [Operational Deployment](docs/whitepaper/operational-deployment.md) | Hardening, tuning, and air-gapped deployment procedures |
| [Distributed Worker Architecture](docs/architecture/distributed-worker-architecture.md) | Hash-ring replication and partition synchronization spec |
